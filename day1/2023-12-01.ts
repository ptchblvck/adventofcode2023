...
const calibrationStringLineArray = calibrationStringArray.map((stringLine) => {
  //   console.log(stringLine);
  return stringLine.split("");
});

const twoDigitNumberArray: string[] = [];

for (const line of calibrationStringLineArray) {
  const firstDigit = line.find((char) => {
    if (char.match(/[0-9]/)) {
      return parseInt(char);
    }
  });
  const lastDigit = line.reverse().find((char) => {
    if (char.match(/[0-9]/)) {
      return parseInt(char);
    }
  });
  const twoDigitNumber = `${firstDigit}${lastDigit}`;
  twoDigitNumberArray.push(twoDigitNumber);
}

const twoDigitNumberArrayNumber = twoDigitNumberArray.map((number) => {
  return parseInt(number);
});

const sum = twoDigitNumberArrayNumber.reduce((acc, curr) => {
  return acc + curr;
});
